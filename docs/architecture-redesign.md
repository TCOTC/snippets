...
当前工作区：**干净**（b206b50 已提交；阶段 3（sync 收敛）、阶段 4（config 声明式 + 拆分瘦身）收官；index.ts 已由 ~6200 行降至 ~727 行。阶段 4 各批外迁细目、Host 注入根除与保留门面清单见下方「依赖注入收敛」与「已建模块」小节；阶段 5（UI 视图化）、阶段 6（jcsm 收敛）见「下一步建议」）。
...
- **index.ts 当前规模 ~727 行**（原 ~6200）：主体为生命周期/装配（onload 各服务一行 `new Xxx(this)`；onLayoutReady 的 sync 分发注册表已外迁 `SnippetManager.buildSyncHandlers()`（b206b50），仅剩 BroadcastService 装配与 start）、initConfigItems ctx 装配、配置镜像 declare + 状态访问器（jcsm 读写）、syncService/console 等少量门面。各 Host 模块也已全部转为同一直连形态。
...
4. （b206b50）跨窗口广播业务分发注册表（原 onLayoutReady 内联）外迁至 `SnippetManager.buildSyncHandlers()`：注册键内联来源解析后调同一方法并传 origin 为 "remote"，onLayoutReady 仅剩 BroadcastService 装配（logger + handlers 来源）与 start。index.ts 降至 ~727 行，主体仅剩生命周期/装配/schema ctx/镜像 declare/门面。
5. 之后：UI 视图化（阶段 5）、jcsm 收敛（阶段 6，前置已启动：见上方「jcsm 现状」节）。
