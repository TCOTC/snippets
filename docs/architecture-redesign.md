...
当前工作区：**干净**（2f399d5 已提交；阶段 3（sync 收敛）、阶段 4（config 声明式 + 拆分瘦身）收官；index.ts 已由 ~6200 行降至 ~685 行。阶段 4 各批外迁细目、Host 注入根除与保留门面清单见下方「依赖注入收敛」与「已建模块」小节；阶段 5（UI 视图化）、阶段 6（jcsm 收敛）见「下一步建议」）。
...
- **index.ts 当前规模 ~685 行**（原 ~6200）：主体为生命周期/装配（onload 各服务一行 `new Xxx(this)`；onLayoutReady 的 sync 分发注册表已外迁 `SnippetManager.buildSyncHandlers()`（b206b50），仅剩 BroadcastService 装配与 start）、initConfigItems ctx 装配、配置镜像 declare + 状态访问器（jcsm 读写）、syncService/console 等少量门面。reloadUI（遍历打开的编辑对话框检查未保存变更 + 二次确认）已外迁 `SnippetsDialog.reloadUI()`（2f399d5），`postReloadUI` 保留插件类门面（menu/file-watch/snippets-dialog 5 处直连）。各 Host 模块也已全部转为同一直连形态。
...
5. （2f399d5）`reloadUI` 外迁至 `SnippetsDialog.reloadUI()`（同域：编辑对话框集合与确认框均在 snippets-dialog.ts）：菜单重载按钮（menuClickHandler data-type=reload）与命令注册改直连 snippetsDialog；`postReloadUI` 保留插件类门面（menu/file-watch/snippets-dialog 5 处直连）。index.ts 降至 ~685 行，主体仅剩生命周期/装配/schema ctx/镜像 declare/门面。
6. 之后：UI 视图化（阶段 5）、jcsm 收敛（阶段 6，前置已启动：见上方「jcsm 现状」节）。
