...
当前工作区：**干净**（694bd0f 已提交；阶段 3（sync 收敛）、阶段 4（config 声明式 + 拆分瘦身）收官；index.ts 已由 ~6200 行降至 ~662 行。阶段 4 各批外迁细目、Host 注入根除与保留门面清单见下方「依赖注入收敛」与「已建模块」小节；阶段 5（UI 视图化）、阶段 6（jcsm 收敛）见「下一步建议」）。
...
- **index.ts 当前规模 ~662 行**（原 ~6200）：主体为生命周期/装配（onload 各服务一行 `new Xxx(this)`；onLayoutReady 的 sync 分发注册表已外迁 `SnippetManager.buildSyncHandlers()`（b206b50），仅剩 BroadcastService 装配与 start；顶栏按钮创建/打开回调已外迁 `SnippetsMenu.initTopBar()`/`openSnippetsManager()`（694bd0f））、initConfigItems ctx 装配、配置镜像 declare + 状态访问器（jcsm 读写）、syncService/console 等少量门面。reloadUI（遍历打开的编辑对话框检查未保存变更 + 二次确认）已外迁 `SnippetsDialog.reloadUI()`（2f399d5），`postReloadUI` 保留插件类门面（menu/file-watch/snippets-dialog 5 处直连）。各 Host 模块也已全部转为同一直连形态。
...
- `src/ui/menu.ts`（阶段 4：**新风格——直连插件实例，无 Host**。`SnippetsMenu(plugin)` 承接原「顶栏菜单」分节整体：open/initSnippetsContainer/setMenuPosition/closeMenuCallback/scrollToMenuItem/menuClickHandler/拖拽排序组（mouse+touch）/filterSnippetsIds/isShowPublishCheckbox/genMenuSnippetsItems/setMenuSnippetsType/setMenuSnippetCount/setMenuSelection/clearMenuSelection/setReloadUIButtonBreathing/setSnippetsTypeSwitchBreathing/编辑按钮高亮组 + 菜单状态字段（menu/menuItems/isDragging/呼吸标志）；随后全局键盘协调组（globalKeyDownHandler/destroyGlobalKeyDownHandler/isDialogAndMenuOpen）也随迁本类（9c8f008），顶栏按钮创建/打开回调（initTopBar/openSnippetsManager，694bd0f）同迁，SettingDialog/ListenerRegistry/SnippetsDialog host 相应改指 menuView。index 仅保留镜像属性 declare 与访问器；菜单打开与菜单项渲染相关调用点（schema ctx/snippets_sort/SNIPPETS_CHANGED/生命周期 close）直连 menuView）
...
- `src/ui/snippets-dialog.ts`（阶段 4：**新风格——直连插件实例，无 Host**。`SnippetsDialog(plugin)` 承接原「对话框相关」分节：公开 `genEditDialogHtml`/`openEditDialog`/`openDeleteDialog`/`openCancelDialog`/`openConfirm`/`closeByElement`/`getAllModalElements`，随后 `reloadUI`（扫描打开的编辑对话框未保存变更 + 二次确认后重载界面，原 index 方法外迁，2f399d5）同迁本类；index 对话框薄壳已删除、调用点直连 snippetsDialog）
...
6. （694bd0f）顶栏按钮创建与打开回调外迁至 `SnippetsMenu.initTopBar()`/`openSnippetsManager()`（顶栏按钮即菜单入口，schema ctx initTopBar/生命周期装配/命令注册改直连 menuView）。index.ts 降至 ~662 行，主体仅剩生命周期/装配/schema ctx/镜像 declare/门面。
7. 之后：UI 视图化（阶段 5）、jcsm 收敛（阶段 6，前置已启动：见上方「jcsm 现状」节）。
